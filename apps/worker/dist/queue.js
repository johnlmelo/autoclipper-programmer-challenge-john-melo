...
export const worker = new Worker("render-queue", async (job) => {
...
worker.on("failed", async (job, error) => {
    ...
    if (hasRetryRemaining) {
        await db.query("UPDATE renders SET status = 'queued', error = $1 WHERE id = $2", [String(error), renderId]);
        return;
    }
    await db.query("UPDATE renders SET status = 'failed', completed_at = NOW(), error = $1 WHERE id = $2", [String(error), renderId]);
});
